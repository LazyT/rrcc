...
void zonesDialog::on_buttonBox_clicked(QAbstractButton *button)
{
	...
	if(buttonBox->standardButton(button) == QDialogButtonBox::Save)
	{
		...
		else
		{
			...
			reinterpret_cast<MainWindow*>(parent())->menu_map_zones->actions().at(index)->setText(comboBox_zone->currentText());

			reinterpret_cast<MainWindow*>(parent())->cfg.zones.data()[index].label = comboBox_zone->currentText();
			reinterpret_cast<MainWindow*>(parent())->cfg.zones.data()[index].x1 = spinBox_x1->value();
			reinterpret_cast<MainWindow*>(parent())->cfg.zones.data()[index].y1 = spinBox_y1->value();
			reinterpret_cast<MainWindow*>(parent())->cfg.zones.data()[index].x2 = spinBox_x2->value();
			reinterpret_cast<MainWindow*>(parent())->cfg.zones.data()[index].y2 = spinBox_y2->value();
			reinterpret_cast<MainWindow*>(parent())->cfg.zones.data()[index].times = spinBox_times->value();
		}
	}
	else if(buttonBox->standardButton(button) == QDialogButtonBox::Discard)
	{
		...
		{
			reinterpret_cast<MainWindow*>(parent())->cfg.zones.removeAt(index);
			reinterpret_cast<MainWindow*>(parent())->menu_map_zones->removeAction(reinterpret_cast<MainWindow*>(parent())->menu_map_zones->actions().at(index));
